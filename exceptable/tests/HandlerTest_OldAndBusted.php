 ...
 */
// @todo This test is out of date
abstract class HandlerTest_OldAndBusted extends TestCase {
  ...
  public function testHandleExceptionFailure() : void {
    ...
    $this->expectThrowable(
      (ExceptableFault::UncaughtException)([], $e),
  ...
  public function testLogsFault() : void {
    $handler = new Handler(new Options());
    ...
    $handler->handleFault($code, $message, __FILE__, __LINE__);
  ...
  public function testLogsDebugFault() : void {
    $handler = new Handler(new Options());
  ...
  public function testRegister() : void {
    ...
    $this->assertEquals(
      ...
      "register() did not register Handler->handleFault()"
    );
    $this->assertSame(
      -1,
      $registered["set_error_handler"][1][1],
      "register() did not register Handler->handleFault() for all error types"
    );
   ...
   * @param bool      $success
   */
  protected function assertFaultHandled(int $i, array $error, bool $success) : void {
   ...
   *
   */
  protected function assertFaultLogged(
    Handler $handler,
    int $i,
    array $error,
    bool $success
   ...
   *
   * @param int    $c Fault code
   * #param string $m Fault message
   * @return callable
   */
